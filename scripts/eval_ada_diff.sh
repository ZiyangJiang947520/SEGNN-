...
###
 ...
 # @LastEditTime: 2023-02-06 12:39:48
...
manner=Ada_GD_Diff
for hyper_Diff in 0. 0.1 1.0 10.0; do
for gamma in 0. 0.1 0.5 1.0 5.0; do
for dataset in cora flickr reddit2 arxiv; do ### cora flickr reddit2 arxiv
for model in mlp; do ###gcn sage mlp
    ...
    CUDA_VISIBLE_DEVICES=$1 python ./eval.py \
        ...
        --gamma ${gamma} \
        --criterion ${criterion} 2>&1 | tee ${output_dir}/${dataset}/${manner}/${model}_${criterion}_eval_hyper_Diff=${hyper_Diff}_gamma=${gamma}.log
done
# wait
done
wait
