...
def align_graphs(graphs: List[Data],
    ...
    :param graphs: a list of PyTorch Geometric Data objects
    ...
    for graph in graphs:
        ...
        adj_matrix = torch.zeros((num_i, num_i))
        edge_index = graph.edge_index.numpy()
        adj_matrix[edge_index[0], edge_index[1]] = 1
        adj_matrix = adj_matrix.numpy()

        node_degree = 0.5 * np.sum(adj_matrix, axis=0) + 0.5 * np.sum(adj_matrix, axis=1)
...
def split_class_graphs(dataset):
    ...
    class_graphs = {label: [] for label in unique_labels}

    for label in unique_labels:
        indices = torch.tensor([i for i, y in enumerate(y_list) if y == label], dtype=torch.long)
        subgraph = data.subgraph(indices)
        class_graphs[label].append(subgraph)
